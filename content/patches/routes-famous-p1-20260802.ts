...
export const patchRoutes: Route[] = [
  // ── 武夷浅尝（名景；九曲竹筏+茶馆，勿硬爬）──────────────────
  {
    ...
    introduction:
      '父母搜「武夷」应对上竹筏观峰，而不是特种兵爬天游。九曲溪坐筏看玉女峰，岩茶产区气质用茶馆歇脚补上——大红袍/肉桂浅尝即可，不必追「大师课」。\n\n山路徒步与天游峰台阶整段可删。与厦门鼓浪屿、土楼廊地理远，分两次出门；福州可作进出缓冲但不塞三坊七巷同日。',
    seasonGuide:
  ...
  // ── 普陀 / 舟山────────────────────────────────────────────
  {
    ...
    introduction:
      '普陀是舟山海上观音道场，父母线看的是普济/法雨平缓寺院与岛上慢住，不是佛顶山硬爬特种兵。船渡与排队单独成线更稳；一天一寺或一沙滩。\n\n素斋与清淡海鲜交替；晕船备药。宁波天一阁请走宁波专线；本廊不塞东钱湖。',
    seasonGuide:
  ...
  // ── 开平碉楼（粤西；与丹霞分线）────────────────────────────
  {
    ...
    introduction:
      '开平碉楼是侨乡防御与中西合璧建筑的名景：自力村外观为主，碉楼内部楼梯窄陡，腿脚不便以外观与田野平路即可。本线只深挖一村群，避免一日四村落；锦江里可选可删。\n\n粤菜清蒸鱼/白切鸡保底，少油炸。田埂雨后泥滑穿防滑鞋。韶关丹霞在粤北，地理过远另开专线。',
    seasonGuide:
      '秋冬春舒适。夏湿热缩短户外，多亭廊歇脚。',
    whyFast: '只留自力村一日半也成立。',
    notices: [
  ...
  // ── 韶关丹霞（粤北；与开平分线）────────────────────────────
  {
    ...
    introduction:
      '韶关丹霞山是粤北红崖名景，与开平侨乡分属两端、与张掖七彩丹霞名称易混——本卡写清「韶关」。爸妈以观光车+观景台坐看红崖为主，长线栈道与峡谷硬走量力可删。\n\n粤北菜清蒸河鲜、清汤面保底，少油辣。防晒防滑；与开平、桂林勿同短假硬拼。',
    seasonGuide:
      '秋冬春宜。夏湿热雷雨，缩短徒步。',
  ...
  // ── 千岛湖（杭淳关系写清，勿硬串黄山）──────────────────────
  {
    ...
    introduction:
      '千岛湖常被塞进「杭黄一日游」——对约60不友好。本线独立立项：住淳安电梯湖景房，半天精华游船足够，登岛多台阶可整段删。湖鱼清蒸少油炸。\n\n若与杭州衔接：西湖慢住结束后另日西行，中间留空白；不排黄山索道同周。',
    seasonGuide:
  ...
  // ── 武隆独立──────────────────────────────────────────────
  {
    ...
    introduction:
      '父母搜「武隆」应有独立产品。本线以天生三桥为核心——喀斯特天坑群的电梯下坑观景，看天龙/青龙桥即返，不必走完全程；仙女山与其他溶洞默认删除。\n\n重庆武隆的地质奇观与影视取景地气质：坑底缓坡防滑，雨天慎入；火锅改清油微辣。天坑不是「多走几步更好看」，而是量力即返、留体力回城。若已走重庆主城慢住，可把本线作加段；反之不必强制先住解放碑。\n\n高铁直达武隆可省主城中转；结束后经重庆飞京，留缓冲勿当日连飞。',
    seasonGuide:
  ...
  // ── 黄果树独立────────────────────────────────────────────
  {
    ...
    introduction:
      '搜「黄果树」应直达瀑布产品。本线只写大瀑布观光车+扶梯，镇远与侗寨请走既有黔东线；天星桥/水帘洞台阶多，默认可删。\n\n中国最大瀑布群之一的喀斯特水景：夏秋水量大更壮观也更湿滑，穿防滑鞋备雨衣；至少提前一天「安旅通」分时预约，勿信黄牛票。瀑区即返，不必一日刷完周边溶洞——父母要的是「看见大瀑布」，不是刷完所有支线。\n\n黔味微辣或免辣；安顺/景区电梯酒店歇一夜，次日回贵阳飞京，勿再塞织金洞同日。',
    seasonGuide:
  ...
  // ── 温州 / 雁荡────────────────────────────────────────────
  {
    ...
    introduction:
      '雁荡山以灵峰奇石夜景与观光车减负闻名，父母线不排特种兵爬遍灵岩——灵岩长线台阶默认可删，江心屿亦非必去。温州海鲜清蒸、鱼丸汤少油保底；山区弯道慎夜赶。\n\n与普陀同属浙东/南沿海，船渡与盘山勿同周；乐清连住比温州日归更稳。',
  ...
  // ── 惠州西湖──────────────────────────────────────────────
  {
    ...
    introduction:
      '惠州填珠三角东岸城市缺口：西湖环湖平缓适合爸妈，泗洲塔登顶可删，双月湾车程另算默认可删。粤菜清蒸鱼/白切鸡保底，少油炸生冷。\n\n暑热早晚散步；勿与开平丹霞同短假硬拼，本线短假自足。',
