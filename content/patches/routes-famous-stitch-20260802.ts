...
export const patchRoutes: Route[] = [
  ...
  // ── 闽南廊：厦门鼓浪屿 + 土楼（overwrite xiamen id）────────────
  {
    ...
    seasonGuide:
      '冬春避寒最舒适。夏湿热缩短户外、午后回酒店；台风预警减少出海与山路。',
  ...
  // ── 乐山峨眉（overwrite；名景标题强化）────────────────────────
  {
    ...
    introduction:
      '乐山市与峨眉山市车程近，本线一条走完。爸妈以乐山城区电梯酒店为锚；金顶低温风大，可整段删，只留大佛仍是完整名景线。',
    seasonGuide:
  ...
  // ── P0 九寨沟（黄龙可选）────────────────────────────────────
  {
    ...
    introduction:
      '九寨沟不是成都郊游——进出靠航线与盘山接驳，旺季与冬季封沟政策须行前自查官方公告。爸妈以「坐车看海子 + 平缓栈道」为主，不徒步原始森林远端。\n\n沟口住连住，早进早出；诺日朗换乘中心歇脚吃饭。黄龙另票另日，台阶与高反叠加，膝盖/血压不稳默认删除。',
    seasonGuide:
      '秋彩林最美也最挤；春夏青绿可走。冬雪封路/缩线常见，以景区公告为准；雨雪防滑。',
    whyFast: '黄龙整段可删；只日则泽+树正精华栈道也成立。',
    notices: [
      '行前健康评估；心脑血管等高风险者不宜。',
  ...
  // ── P0 凤凰 + 湘西浅挂───────────────────────────────────────
  {
    ...
    introduction:
      '父母搜「凤凰」应有独立产品，而不是张家界可选尾巴。本线以凤凰古城电梯江景房连住，白天江边慢走，晚上看灯早歇。\n\n芙蓉镇作为湘西地理挂接可选；张家界峰林请走武陵源专线，本廊不塞三索一梯。',
    seasonGuide:
  ...
  // ── P1 庐山牯岭────────────────────────────────────────────
  {
    ...
    introduction:
      '牯岭是山上小镇，电梯酒店连住比日登日返轻松。花径、如琴湖、博物馆类平缓点优先；含鄱口等观景台量力，五老峰默认不排。',
    seasonGuide:
  ...
  // ── P1 绍兴（可挂杭州）──────────────────────────────────────
  {
    ...
    introduction:
      '绍兴古城石板与河道为主，爸妈选近东湖或鲁迅故里电梯酒店。安昌古镇等可选一日；杭州作为进出缓冲，不硬塞乌镇西塘（见江南水乡专线）。',
  ...
  // ── P1 五台山适老减负────────────────────────────────────────
  {
    ...
    introduction:
      '晋北线曾明确勿再塞五台长线——本产品单独立项减负版。爸妈以台怀核心寺庙平路为主，索道与爬山默认可删。\n\n与大同云冈、平遥分线走，中间回京休整。',
    seasonGuide:
