...
export const patchRoutes: Route[] = [
  ...
  // ── 阳朔 ──────────────────────────────────────────────────
  {
    ...
    introduction:
      '阳朔比桂林市区更适合长居：尺度小、山水近、步行可控。避开暑热暴雨与西街酒吧嘈杂带，选稍离核心街的电梯民宿/公寓，把漓江当日常窗外景色而非打卡清单。\n\n喀斯特峰丛与田园是本月主调：漓江游船观景、遇龙河竹筏短段、十里画廊电瓶车、兴坪或杨堤半日。攀岩、特种兵骑行与夜场默认不排。\n\n桂林只作进出港与就医下撤；阳朔镇区菜市场与诊所先踩点。雨季多空白茶馆日，暑期错峰订船票。',
    seasonGuide:
      '春秋最舒适。夏季湿热与暴雨；冬季阴冷潮湿，体感偏凉。台风外围影响偶发，关注预警。',
    notices: [
      '【医疗】本地无三甲：县医院就近；三甲须下撤桂林（约1–1.5小时）。',
      '游船/竹筏选正规票点，备救生衣与防滑鞋。',
      '雨后山路湿滑，日归改室内或茶馆。',
      '桂林两江机场往返预留堵车与接驳时间。',
      '短租先一周试住；避开西街夜噪带。',
  ...
  // ── 镇远 ──────────────────────────────────────────────────
  {
    ...
    introduction:
      '黔东南适合「避暑+古城散步」型长居，不是豪华医疗度假。天气多变、昼夜温差，备外套与防滑鞋。医疗弱于省会——不适即高铁下撤贵阳三甲，心理预案要先写进行程。\n\n江城尺度：舞阳河游船、青龙洞外观、沿江平缓石板街撑起日常；雷山/西江等高坡苗寨特种兵默认缩短或删除。尊重苗侗习俗与拍摄规定，文化浅尝半日即可。\n\n辣酸口味改清淡清蒸；雨季关注山洪预警，夜间少赶石板路。',
    seasonGuide:
      '春、秋、夏（避暑）可排。冬季阴冷潮湿，体感不适则缩短。雨季关注山洪与滑坡预警。',
    notices: [
      '【门槛告警】本地无三甲、物资偏县城级；复杂病症经高铁下撤贵阳三甲。慢病家庭优先改昆明/大理。',
      '古城石板湿滑，夜间少赶路。',
      '自驾山区弯道谨慎，优先高铁+当地交通。',
      '民族村寨尊重当地习俗与拍摄规定。',
      '高坡苗寨特种兵不排；短租问清防潮与热水。',
