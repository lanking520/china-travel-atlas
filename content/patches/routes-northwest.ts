...
export const patchRoutes: Route[] = [
  ...
  // ── 青藏 · 青海湖 ──────────────────────────────────────────
  {
    ...
    summary:
      '西宁约2260米先适应2–3日，再上青海湖选段（湖面约3200米）。包车分段看二郎剑/黑马河，不硬全日环湖、不垫两周空日。更短浅段见 leg-xining-qinghai-lake；东接河西见青甘组合。',
    whyFast:
    ...
    stops: [
      ...
      },
      {
        ...
        lng: 100.136,
        summary:
          '中国最大咸水湖。包车选段停观景台，二郎剑乘观光车，湖边短停即走；中间可插空白半日。',
        tips:
          '湖面约3196–3260米，须在西宁适应后再去。旺季「青海湖景区」公众号提前分时预约，现场常无散票。紫外线极强：帽、墨镜、防晒、薄羽绒（湖风大）。心脏病/未控制高血压慎往；头痛呕吐加重立即下撤回西宁。别一日硬环约360公里。',
        image:
