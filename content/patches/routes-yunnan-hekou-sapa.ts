...
export const patchRoutes: Route[] = [
  // ── 河口边贸腿 ─────────────────────────────────────────────
  {
    ...
    summary:
      '中国境内侧短线：河口口岸县城慢住、老码头浅逛。本腿不越境；接沙巴（Sa Pa）见跨境短线或中越组合卡。',
    whyFast: '昆明缓冲与老码头可删；只河口歇两晚也成立。',
    introduction:
      '河口是中越边境口岸县城，本短线只写中国境内侧：电梯酒店连住、老码头浅逛、湿热午休——不越境、不复写签证通关正文。\n\n若继续去沙巴：证件与通关见「沙巴·镇区慢住」；串线节奏见「河口→沙巴 · 中越慢廊」。',
    seasonGuide:
  ...
  // ── 沙巴跨境腿 ─────────────────────────────────────────────
  {
    ...
    summary:
      '越南侧目的地短线：沙巴（Sa Pa）梯田镇区慢住。须护照与合法签证/入境许可；只走正式口岸。本廊道证件诚实正文在此。',
    whyFast: 'Fansipan 与长距村寨徒步可删；最短：正式通关→沙巴慢住→原路返回。',
    introduction:
      '沙巴（Sa Pa）是越南北部高原小镇（约1500m 级，比河口凉），不是中国国内游。本短线从河口—老街正式通关写起，并承载本廊道证件诚实正文：镇区电梯慢住看梯田云雾，Fansipan 仅正规缆车量力可选。\n\n行前必须自行核实：中国公民护照有效期、越南签证/电子签/免签政策（中国领事服务网与越南主管机关最新公告）。只走正式口岸。语言、货币、通信与境外医疗保险与国内不同；重症计划经正式口岸撤回昆明三甲。可单订，或先订河口中国段再接本短线 / 中越组合。',
    seasonGuide:
  ...
  // ── 中越 compose ───────────────────────────────────────────
  {
    ...
    introduction:
      '本卡是「短线串起来」的长线组合，不是第三份沙巴游记：① 河口中国段 → ② 白昼正式口岸 glue → ③ 沙巴越南段。景点与通关细则见各短线；本卡只管顺序与衔接。\n\n跨境须护照与合法越南入境许可，只走正式口岸（诚实正文见沙巴短线）。可只订河口段不出境。',
    researchKeywords: [
      '河口口岸 老街 通关',
      '沙巴 Sapa 父母 慢游',
      '河口 沙巴 中越',
