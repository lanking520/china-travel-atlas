...
export const patchRoutes: Route[] = [
  // ── 河口边贸腿 ─────────────────────────────────────────────
  {
    ...
    summary:
      '独立短线：昆明缓冲后河口口岸县城慢住，红河沿岸与老码头浅逛。本腿不越境；若要沙巴须另订跨境短线/组合，且只走正式口岸。',
    whyFast: '昆明缓冲与老码头可删；只河口歇两晚也成立。',
    introduction:
      '河口是中越边境口岸县城，本短线只写中国段：电梯酒店、老码头浅逛、熟悉口岸动线。\n\n不提供任何非正式通道建议。出境去越南沙巴须护照与合法入境许可，见「沙巴·镇区慢住」短线与中越组合卡。',
    seasonGuide:
      '秋冬春相对适宜。暑期湿热；台风/强降雨预警期减少户外。',
    notices: [
      '本腿止于中国境内侧；不越境、不旁路。',
      '若计划出境：行前自查护照与越南入境政策（领事服务网+越方公告）。',
      '确认口岸开放时段；防滑防晒。',
    ],
    researchKeywords: ['河口口岸 父母', '昆明 河口 火车', '河口 老码头'],
    sources: [
      {
  ...
  // ── 沙巴跨境腿 ─────────────────────────────────────────────
  {
    ...
    summary:
      '跨境短线：沙巴在越南境内——须护照与合法签证/入境许可。镇区电梯酒店慢住，梯田观景为主；Fansipan 仅缆车量力可选。不提供偷渡建议。',
    whyFast: 'Fansipan 与长距村寨徒步可删；最短：正式通关→沙巴慢住→原路返回。',
    introduction:
      '沙巴（Sa Pa）为越南北部高原小镇，约1500m 级，比河口凉。本短线从正式通关写起，到镇区慢住与可选缆车。\n\n行前必须自行核实：中国公民护照有效期、越南签证/电子签/免签政策（中国领事服务网与越南主管机关最新公告）。只走河口—老街正式口岸。语言、货币、通信与境外医疗保险与国内不同；重症计划经正式口岸撤回昆明三甲。',
    seasonGuide:
  ...
  // ── 中越 compose ───────────────────────────────────────────
  {
    ...
    coverImage:
      'https://upload.wikimedia.org/wikipedia/commons/thumb/2/20/Sa_Pa_Rice_Terrace_I.jpg/1280px-Sa_Pa_Rice_Terrace_I.jpg',
    summary:
      '长线组合卡：嵌入河口边贸、沙巴跨境两条短线。沙巴在越南——须护照与合法签证；只走正式口岸。景点正文见各短线。Fansipan 徒步默认可删。',
    whyFast:
