...
0. **小红书是必要调研环境**（全国城市攻略/图文/视频极丰富）。通过本机 [XHS MCP](./XHS-MCP使用说明.md) 搜索与读帖，再摘要进 `notes/`。  
1. **先搜后写**：用小红书 / 知乎 / 官网搜到的信息，改写成活跃退休短句写入 `content/routes.ts`。  
2. **不进网页包**：原始帖文、导出 JSON、Cookie 一律放 `raw/` 或 `~/.xiaohongshu/`，站点构建不读这里。  
3. **不整篇搬运**：平台原文只作素材；公开站只放我们改写后的内容 + 可选来源链接。  
4. **合规**：登录态仅本机；勿把逆向爬虫接到 CI 或父母站点。
...
整理好 `notes/<id>.md` 后，可把要点贴给 AI，要求输出符合 `content/types.ts` 的字段补丁（活跃退休口吻、勿抄原文）。
