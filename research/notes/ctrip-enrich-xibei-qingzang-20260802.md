...
- `content/audit-patches/ctrip-enrich-xibei-qingzang-20260802.ts` — `detailPatches` (introduction / seasonGuide / notices) + `routeFieldPatches.sources`
- Registered last in `content/audit-patches/index.ts` so richer intros win via `preferRicherText`
...
Note: `routeFieldPatches.sources` **replaces** prior `sources` arrays on these ids (Ctrip/official enrich set). Wikivoyage-only entries on the same ids may need re-merge in a later hygiene pass if both must show.
